...
export default class Twitter extends Extractor {
  ...
  get hasKeys() {
    return !!process.env.TWITTER_CONSUMER && !!process.env.TWITTER_SECRET;
  }

  async refreshToken() {
    const response = await needle('post', 'https://api.twitter.com/oauth2/token', 'grant_type=client_credentials', {
      headers: {
        Authorization: `Basic ${Buffer.from(
          process.env.TWITTER_CONSUMER + ':' + process.env.TWITTER_SECRET,
          'binary'
        ).toString('base64')}`
      }
    });
    ...
    if (!this.tweetIDs.has(key)) {
      ...
      if (response.statusCode === 404) return;
        ...
        if (twitterData && twitterData.extended_entities && twitterData.extended_entities.media) {
          const media = twitterData.extended_entities.media[mediaID] || twitterData.extended_entities.media[0];
